...
func NoteGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + noteTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + noteTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
func NoteAdd(w http.ResponseWriter, r *http.Request) {
	...
	fieldCheck := requiredFiledsCheck(body, noteRequiredFields)
	if len(fieldCheck) > 0 {
	...
	// log
	logAction(body["admin_name"], "added note", "5", body["hostel_id"])
	delete(body, "admin_name")

	body["status"] = "1"
	body["created_date_time"] = time.Now().UTC().String()

	status, ok := insertSQL(noteTable, body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Note added", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
func NoteUpdate(w http.ResponseWriter, r *http.Request) {
	...
	// log
	logAction(body["admin_name"], "updated note", "5", body["hostel_id"])
	delete(body, "admin_name")

	body["modified_date_time"] = time.Now().UTC().String()

	status, ok := updateSQL(noteTable, r.URL.Query(), body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Note updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
