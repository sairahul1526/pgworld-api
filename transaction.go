...
func Rent(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		if len(body["bill_id"]) == 0 {
			...
			insertSQL(billTable, map[string]string{"hostel_id": body["hostel_id"], "user_id": body["user_id"], "title": body["title"], "description": body["description"], "amount": body["amount"], "paid_date_time": body["paid_date_time"], "status": "1", "paid": "0", "document": body["document"], "type": body["type"], "payment": body["payment"], "transaction_id": body["transaction_id"]})
		} else {
			// log
			logAction(body["admin_name"], "updated rent ₹"+body["amount"]+" from "+body["name"], "7", body["hostel_id"])
			updateSQL(billTable, url.Values{"id": {body["bill_id"]}, "hostel_id": {body["hostel_id"]}, "user_id": {body["user_id"]}}, map[string]string{"amount": body["amount"], "paid_date_time": body["paid_date_time"], "document": body["document"], "type": body["type"], "payment": body["payment"], "transaction_id": body["transaction_id"]})
		}
		if strings.EqualFold(body["joining"], "1") {
...
func Salary(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		if len(body["bill_id"]) == 0 {
			...
			updateSQL(billTable, url.Values{"id": {body["bill_id"]}, "hostel_id": {body["hostel_id"]}, "employee_id": {body["employee_id"]}}, map[string]string{"amount": body["amount"], "paid_date_time": body["paid_date_time"], "document": body["document"], "type": body["type"], "payment": body["payment"], "transaction_id": body["transaction_id"]})
		}
		response["meta"] = setMeta(status, "User updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
