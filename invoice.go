...
func InvoiceGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + invoiceTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + invoiceTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
// InvoiceAdd .
func InvoiceAdd(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	fieldCheck := requiredFiledsCheck(body, invoiceRequiredFields)
	if len(fieldCheck) > 0 {
	...
	// log
	// logAction(body["admin_name"], "added invoice", "4", body["invoice_id"])
...
// InvoiceUpdate .
func InvoiceUpdate(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	for _, field := range invoiceRequiredFields {
	...
	status, ok := updateSQL(invoiceTable, r.URL.Query(), body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Invoice updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
